...
9. **交互验证前先确保环境干净**：跑任何 UI 验证之前必须先清 `resume.json`（断点记录）、`pkill md-player` 确认没有残留进程、确认没有模态框开着。曾经因为遗留断点记录让模态续播框一直开着，把「模态框挡住点击 + 按住快捷键失效」整整误判成播控条与快捷键功能回归，白查了十几轮。**现象不对先怀疑环境，再怀疑代码。**
   **「点完下拉框快捷键就全哑」的真正原因是弹层还开着。** 带 `CloseOnEscape` / `CloseOnPressOutside` 的 Popup（ComboBox 弹层就是）会在 overlay 上装按键过滤，把窗口级 `Shortcut` 一并吃掉；此时窗口 `active` 仍是 true、`activeFocusItem` 也可能还是根项，三个量看着全正常，键就是不响。`closePolicy: NoAutoClose` 的 Popup 不吃键——标题·章节面板（深坑 #7 / D-018）开着时快捷键全通，实测 T/空格/方向键各 2/2、1/1（D-062 三态表）。**裸二进制与 .app 在这件事上没有差别**：裸二进制的激活策略 Qt 的 cocoa 插件已经设成 `Regular`（实测 `Regular -> Regular`），`System Events` 也稳定枚举到 `count of windows = 1`，D-010 不受影响、不必为验收去打 .app 包（D-061 作废了此处上一版的机制描述）。**验证前只需确认两件事：窗口是 frontmost（日志打 `Window.active`），以及没有弹层开着**（`MD_LOG_UI` 会打弹层开合）。少看后一个量，必然把「弹层没关」误判成焦点或激活问题——两轮误判都栽在这里。
...
11. **验证方法缺参照系 —— 两种典型，都会给出「没问题」的假结论**：
    (a) **阴性结论未自检工具**。「命中 0 / 无匹配」当证据之前，先确认工具真的看得见目标。实例：本仓库的 `grep` 是个 shell 函数包装，对 ISO-8859 等非 UTF-8 文件**静默跳过**，于是 `list.h` 的引用数被数成 5（实际 0）、`vendor/dstdec/` 的许可头被数成 6/14（实际 14/14），两处结论都反了。校验办法：换一个独立工具（`awk` / `sed` / Python）复算一遍，或先用一个**必然命中**的模式自检工具本身。另一个同类实例：验「点完下拉框快捷键还灵不灵」时合成点击的坐标写错（点在画面上，框实际在别处），弹层**一次都没打开**，于是连跑数轮全部「验证通过」——测试根本没碰到被测对象。凡是「操作 → 观察」的验证，先让**操作本身**在日志里留下证据（这里是给弹层开合加常驻探针），再看结论。
    ...
    (c) **测量手段把被测现象本身消掉了**。实例：用 `ao=pcm` 录音查拖动杂音——同一次拖动，真实 CoreAudio 下有 89 次 `starting audio playback`，`ao=pcm` 下只有 2 次，因为没有设备要喂就不会刷 AO。录音干干净净，而缺陷就在被消掉的那部分里。**选参照物之前先问一句：这个测法会不会把要测的东西本身消掉？** 会，就换测法——补一份「修后」的同样干净的录音只会让两边都干净被读成「修好了」。
    (d) **拿延迟论证噪声**。实例：用「每次 seek 到位 0–14 ms」得出「拖动不是问题」——快不等于不响，30 次/秒的快重启正是噪声本身。
    (e) **测了相邻的量，没测到点上**。实例：查「seek 慢」时做了 ffmpeg 输入 seek 对照，却只测**样本相同率**（100%，证明的是正确性），没测**耗时**；真正要的那个量隔了一轮才补上（0.02 s，一测就知道 demuxer 不背这个锅）。
