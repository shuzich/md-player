...
7. **Qt Quick Popup 的焦点陷阱**：Popup（含 Drawer/Dialog）设 `focus: true` 后，`QQuickPopupItem` 会成为窗口的 `activeFocusItem`，此后**所有 `Qt.WindowShortcut` 快捷键静默失效**——`enabled` 仍是 `true`，就是不触发，没有任何报错。本项目所有覆盖在播放画面上的 Popup 一律 `focus: false`，Esc 关闭改由窗口级 `Shortcut` 承担；面板内部的 Controls 控件还须显式 `focusPolicy: Qt.NoFocus`，否则点一下控件同样会把焦点吸进 Popup。详见 D-018。
   ...
   **第三条路：吞掉窗口级快捷键的不只有焦点，还有 Popup 的 `closePolicy`。** ComboBox 弹层默认的 `CloseOnEscape | CloseOnPressOutside` 会让**所有** `Qt.WindowShortcut` 静默失效，而此时窗口 `active` 是 true、`activeFocusItem` 还停在根项——照深坑 #7 原来的两个量去查，三个量看着全正常，必然查错方向。判别办法很便宜：**把该 Popup 的 `closePolicy` 临时改成 `Popup.NoAutoClose` 跑一遍**，快捷键回来了就是它。责任已缩到 `CloseOnEscape` 一位上（`NoAutoClose` 与只留 `CloseOnPressOutside` 两种配置下快捷键都正常，加回 `CloseOnEscape` 就哑）。本项目三个下拉框因此只留 `CloseOnPressOutside`，Esc 由窗口级 `Shortcut` 按「弹层 → 面板」的顺序派发。详见 D-062。
