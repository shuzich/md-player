...
9. **交互验证前先确保环境干净**：跑任何 UI 验证之前必须先清 `resume.json`（断点记录）、`pkill md-player` 确认没有残留进程、确认没有模态框开着。曾经因为遗留断点记录让模态续播框一直开着，把「模态框挡住点击 + 按住快捷键失效」整整误判成播控条与快捷键功能回归，白查了十几轮。**现象不对先怀疑环境，再怀疑代码。**
   **「点完下拉框快捷键就全哑」的真正原因是弹层还开着，不是激活、也不是焦点。** 任何一个打开的 Popup 都会吞掉全部 `Qt.WindowShortcut`，此时窗口 `active` 仍是 true、`activeFocusItem` 仍是根项——三个量全部正常，键就是不响。同一次运行内的对照：弹层开着时 T×2 / 空格×2 全无反应，点画面把弹层 dismiss 掉后同样的键 2/2、2/2 全中。裸二进制与 .app 在这件事上没有差别（裸二进制的激活策略 Qt 自己已经设成 `Regular`，启动时打一行就能读出来），D-010 不受影响。分辨办法是把**三个量一起**打出来：`Window.active` / `activeFocusItem` / **弹层是否开着**——只看前两个必然查错方向，两轮误判都栽在这里（D-061 取代 D-058 的相应表述）。
