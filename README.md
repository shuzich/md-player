...
| `空格` | 暂停 / 继续 |
| `←` / `→` | ±5 秒 |
| `Ctrl+←` / `Ctrl+→` | ±60 秒。**macOS 上实际要按 `⌘←` / `⌘→`**——Qt 在 macOS 把 `Ctrl` 映射成 Command（Qt 的既定行为，不是本项目的键位选择），按 Ctrl 完全没反应 |
| `↑` / `↓` | 音量 ±5 |
| `m` | 静音 |
| `s` / `Shift+S` | 截图（纯画面 / 含字幕） |
| `t` | 标题·章节面板（碟类资源） |
| `g` | SACD +6dB 增益开关。设置页里也有同一项，**两者并存且双向同步**——G 是 D-059 第 1 项（+6dB A/B 是否可闻）人工验收的仪器，那一项关闭前不收回（D-070） |
| `,` | 设置页（播控条右端的「设置」按钮是等效入口） |
...
| 项 | 说明 |
|---|---|
| 硬件解码 | 关掉后强制软解，排查花屏/黑屏时用。macOS 上开启时 `hwdec-current` 为 `videotoolbox-copy`；**`bd://` 上本来就不走硬解，读到 `no` 是正常的** |
| 独占音频输出 | 独占声卡，其它应用暂时发不出声。开启时该设备的 `kAudioDevicePropertyHogMode` 会记上本进程 pid |
| 音频直通（比特流） | 把 AC3 / DTS / TrueHD 原样送给功放解码，**需要 HDMI 或光纤**。内置扬声器不接受比特流：打开后 1.5 秒内若音频输出没能开起来，会自动关回去并说明；若是设备本来就被别的应用独占，则只提示、不动你的开关（D-070 / D-071） |
| 截图目录 | 默认 `~/Pictures/md-player/`，用系统文件夹选择器改。目录不可写时截图会点名提示，不会静默失败 |
| SACD 增益 +6dB | 补足 DSD 解码后比 foobar2000 低的约 6dB；与快捷键 `G` 同步 |

设置全部**运行时下发**，`configs/mpv-baseline.conf` 不参与（那份是 T2 定下的手感基线，按 D-013 冻结）；
状态落 QSettings，重启保持。**直通的真效果（比特流确实到达功放并被正确解码）本机无法验证**，
已形式化挂账，待有 HDMI / 光纤功放时补验（D-070）。

进度条拖动时贴关键帧即时出画，松手落精确点（CLAUDE.md 深坑 #2）；章节以刻度显示在进度条上。
**纯音频例外**：没有画面时「即时出画面」这条理由不成立，而每秒约 30 次的 AO 重启会把拖动
全程变成一片杂音，所以纯音频（SACD、flac / mp3 / m4a……）拖动过程中一次 seek 都不发，
只动进度条与时间标签，松手才跳一次（D-055）。
播控条上的**音轨 / 字幕下拉框常驻**：只有一条音轨或碟上没有字幕时不隐藏，而是置灰并写明
当前是什么（`音轨：ac3 · 立体声 2ch` / `字幕：无`）——隐藏会让人以为播放器没有这个功能（D-058）。
